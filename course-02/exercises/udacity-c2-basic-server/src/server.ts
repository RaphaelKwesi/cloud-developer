...
(async () => {
  ...
  app.get( "/cars/", ( req: Request, res: Response ) => {
    ...
    // if we have an optional query paramater, filter by that parameter
    if (make) {
      cars_list = cars.filter((car) => car.make === make);
    }

    // return the resulting list along with 200 success status
  ...
  app.get( "/cars/:id", ( req: Request, res: Response ) => {
    ...
    // try to find the car by id (change datatype of id from string to number by using parseInts)
